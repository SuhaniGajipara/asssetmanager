...
const Deliveries = () => {
  const [data, setData] = useState(deliveriesData || []);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center gap-4 border-l-4 border-primary">
          <div className="p-3 bg-primary-container text-primary rounded-full"><Truck size={24}/></div>
          <div><p className="text-sm text-on-surface-variant">Total Deliveries</p><h3 className="text-xl font-bold">{data.length}</h3></div>
        </Card>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Deliveries" footer={<><Button variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button variant="primary" type="submit" form="addForm">Save</Button></>}>
        <form className="space-y-4" onSubmit={handleAddSubmit} id="addForm">
          <Input id="orderId" label="Order ID" value={formData.orderId || ''} onChange={handleInputChange} required /><Input id="destination" label="Destination" value={formData.destination || ''} onChange={handleInputChange} required /><Input id="driver" label="Driver" value={formData.driver || ''} onChange={handleInputChange} required />
        </form>
      </Modal>
    </div>
  );
