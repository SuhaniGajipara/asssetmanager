...
import { Plus, ArrowDownCircle, Eye, Edit, Trash2 } from 'lucide-react';
...
const PurchaseReceipts = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader title="Purchase Receipts" subtitle="Detailed view of received purchases." action={<Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary/90 text-white flex items-center gap-2 px-4 py-2 rounded-md"><Plus size={18} /> Add New</Button>} />

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center gap-4 border-l-4 border-primary">
          <div className="p-3 bg-primary-container text-primary rounded-full"><ArrowDownCircle size={24}/></div>
          <div><p className="text-sm text-on-surface-variant">Total Receipts</p><h3 className="text-xl font-bold">{data.length}</h3></div>
        </Card>
        <Card className="p-4 flex items-center gap-4 border-l-4 border-secondary">
          <div className="p-3 bg-primary-container text-secondary rounded-full"><ArrowDownCircle size={24}/></div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-container border-b border-outline-variant/50 text-primary text-sm">
                <th className="py-3 px-6 font-semibold">PO ID</th><th className="py-3 px-6 font-semibold">Warehouse</th><th className="py-3 px-6 font-semibold">Status</th>
            ...
            <tbody>
              {filtered.map((row, idx) => (
                <tr key={idx} className="border-b border-outline-variant/30 hover:bg-surface-dim transition-colors group">
                  <td className="px-6 py-4 font-semibold text-on-surface">{row[Object.keys(row)[1]] || row.id}</td><td className="px-6 py-4 text-on-surface-variant">{row[Object.keys(row)[2]] || ''}</td><td className="px-6 py-4 text-on-surface-variant">{row[Object.keys(row)[3]] || ''}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Purchase Receipts" footer={<><Button variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button variant="primary" type="submit" form="addForm">Save</Button></>}>
        <form className="space-y-4" onSubmit={handleAddSubmit} id="addForm">
          <Input id="poId" label="PO ID" value={formData.poId || ''} onChange={handleInputChange} required /><Input id="warehouse" label="Warehouse" value={formData.warehouse || ''} onChange={handleInputChange} required />
        </form>
      </Modal>
    </div>
  );
