...
const StockLevels = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader title="Stock Levels" subtitle="View granular stock availability." action={<Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary/90 text-white flex items-center gap-2 px-4 py-2 rounded-md"><Plus size={18} /> Add New</Button>} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center gap-4 border-l-4 border-primary">
          ...
          <div><p className="text-sm text-on-surface-variant">Total Records</p><h3 className="text-xl font-bold">{data.length}</h3></div>
        </Card>
        <Card className="p-4 flex items-center gap-4 border-l-4 border-secondary">
          <div className="p-3 bg-primary-container text-secondary rounded-full"><Layers size={24}/></div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-container border-b border-outline-variant/50 text-primary text-sm">
                <th className="py-3 px-6 font-semibold">Product</th><th className="py-3 px-6 font-semibold">SKU</th><th className="py-3 px-6 font-semibold">Warehouse</th><th className="py-3 px-6 font-semibold">Available</th>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Stock Levels" footer={<><Button variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button variant="primary" type="submit" form="addForm">Save</Button></>}>
        <form className="space-y-4" onSubmit={handleAddSubmit} id="addForm">
          <Input id="product" label="Product" value={formData.product || ''} onChange={handleInputChange} required /><Input id="sku" label="SKU" value={formData.sku || ''} onChange={handleInputChange} required /><Input id="warehouse" label="Warehouse" value={formData.warehouse || ''} onChange={handleInputChange} required /><Input id="available" label="Quantity" value={formData.available || ''} onChange={handleInputChange} required />
        </form>
      </Modal>
    </div>
  );
