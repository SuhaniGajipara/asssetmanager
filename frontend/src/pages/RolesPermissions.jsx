...
const RolesPermissions = () => {
  const [data, setData] = useState(rolesPermissionsData || []);
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader title="Roles & Permissions" subtitle="Configure system roles." action={<Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary/90 text-white flex items-center gap-2 px-4 py-2 rounded-md"><Plus size={18} /> Add New</Button>} />

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center gap-4 border-l-4 border-primary">
          <div className="p-3 bg-primary-container text-primary rounded-full"><Shield size={24}/></div>
          <div><p className="text-sm text-on-surface-variant">Total Roles</p><h3 className="text-xl font-bold">{data.length}</h3></div>
        </Card>
        <Card className="p-4 flex items-center gap-4 border-l-4 border-secondary">
          <div className="p-3 bg-primary-container text-secondary rounded-full"><Shield size={24}/></div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-container border-b border-outline-variant/50 text-primary text-sm">
                <th className="py-3 px-6 font-semibold">Role</th><th className="py-3 px-6 font-semibold">Description</th><th className="py-3 px-6 font-semibold">Users</th>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Roles & Permissions" footer={<><Button variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button variant="primary" type="submit" form="addForm">Save</Button></>}>
        <form className="space-y-4" onSubmit={handleAddSubmit} id="addForm">
          <Input id="role" label="Role Name" value={formData.role || ''} onChange={handleInputChange} required /><Input id="description" label="Description" value={formData.description || ''} onChange={handleInputChange} required />
        </form>
      </Modal>
    </div>
  );
