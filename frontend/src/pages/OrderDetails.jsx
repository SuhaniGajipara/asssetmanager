...
const OrderDetails = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader title="Order Details" subtitle="Granular view of order items." action={<Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary/90 text-white flex items-center gap-2 px-4 py-2 rounded-md"><Plus size={18} /> Add New</Button>} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4 flex items-center gap-4 border-l-4 border-secondary">
          <div className="p-3 bg-primary-container text-secondary rounded-full"><FileText size={24}/></div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-primary-container border-b border-outline-variant/50 text-primary text-sm">
                <th className="py-3 px-6 font-semibold">Order ID</th><th className="py-3 px-6 font-semibold">Customer</th><th className="py-3 px-6 font-semibold">Items</th><th className="py-3 px-6 font-semibold">Status</th>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Order Details" footer={<><Button variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel</Button><Button variant="primary" type="submit" form="addForm">Save</Button></>}>
        <form className="space-y-4" onSubmit={handleAddSubmit} id="addForm">
          <Input id="customer" label="Customer" value={formData.customer || ''} onChange={handleInputChange} required />
        </form>
      </Modal>
    </div>
  );
