...
const Sidebar = () => {
  ...
  const bgClass = isDark ? 'bg-surface-container-lowest border-r border-outline-variant/30' : 'bg-surface-container-lowest border-r border-outline-variant';
  const textClass = 'text-on-surface';
  const textMutedClass = 'text-on-surface-variant';
  const hoverClass = 'hover:bg-surface-dim hover:text-primary';
  const activeClass = 'bg-primary-container text-primary font-semibold border-l-4 border-primary rounded-l-none';
  ...
  return (
    <>
      ...
      >
        ...
        <div className="h-20 flex items-center justify-between px-4 border-b border-outline-variant/50">
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap px-2">
              <Package className="text-primary" size={28} />
              <span className={`text-xl font-bold tracking-tight text-on-surface`}>Asset Manager</span>
            </div>
          )}
          {isSidebarCollapsed && (
          ...
          <button
            ...
            className={`hidden md:flex p-1.5 rounded-full ${hoverClass} text-outline-variant`}
          >
            <ChevronLeft size={20} className={`transition-transform ${isSidebarCollapsed ? 'rotate-180' : ''}`} />
          </button>
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className={`md:hidden p-1.5 rounded-full ${hoverClass} text-outline-variant`}
          >
            <ChevronLeft size={20} />
          </button>
        </div>
        ...
        {/* User Profile */}
        <div className={`p-4 border-b border-primary/30 flex items-center gap-3 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
          <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-primary text-white">
            <User size={20} />
          </div>
          {!isSidebarCollapsed && (
            <div className="overflow-hidden whitespace-nowrap">
              <h2 className="text-sm font-semibold truncate text-on-surface">Admin User</h2>
          ...
          {menuGroups.map((group, idx) => (
            <div key={idx}>
              {!isSidebarCollapsed && group.title && (
                <h3 className="px-3 mb-2 text-xs font-semibold tracking-wider text-outline-variant uppercase">
                  {group.title}
                </h3>
              )}
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        {!isSidebarCollapsed && <span className={`text-sm ${isActive ? 'font-medium' : ''} whitespace-nowrap`}>{item.label}</span>}
                      </>
                    )}
                  </NavLink>
                ))}
        ...
        <div className="p-4 border-t border-primary/30">
          <button
            ...
            className={`flex items-center gap-3 w-full rounded-md transition-colors ${isSidebarCollapsed ? 'justify-center px-0 py-3' : 'px-3 py-2.5'} text-error hover:bg-error/10`}
