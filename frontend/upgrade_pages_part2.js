...
const purchaseContent = `import React, { useState } from 'react';
...
const PurchaseOrders = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader title="Purchase Orders" subtitle="Manage inbound orders from suppliers." action={<Button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-primary/90 text-white flex items-center gap-2 px-4 py-2 rounded-md"><Plus size={18} /> New PO</Button>} />
...
const transfersContent = `import React, { useState } from 'react';
...
const adjustmentsContent = `import React, { useState } from 'react';
...
fs.writeFileSync(path.join(pagesDir, 'PurchaseOrders.jsx'), purchaseContent);
fs.writeFileSync(path.join(pagesDir, 'Transfers.jsx'), transfersContent);
fs.writeFileSync(path.join(pagesDir, 'Adjustments.jsx'), adjustmentsContent);
console.log('Updated Purchase, Transfers, Adjustments');
